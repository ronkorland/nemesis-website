...
reportsApp.controller('TestsController', function($scope, $location, $timeout,
		$modal, $filter, TestService) {
	...
	TestService.getTestsMethodNames().then(function(data) {
		$scope.names = data;
	}, function() {
		$scope.names = [];
	});
	...
	$scope.fetchResult = function() {
		return TestService.getTests($scope.filterCriteria).then(function(data) {
	...
	// Will be called when filtering the grid, will reset the page number to
	// zero
	$scope.filterResult = function() {
		...
		if ($scope.filterCriteria.minusDays == '-1' && !$scope.dpModalOpen
				&& !$scope.filterCriteria.startDate) {
	...
	$scope.openDatepicker = function() {
		...
		var modalInstance = $modal.open({
			templateUrl : 'template/modal/DatePickerModal.html',
			controller : 'ModalDpCtrl'
		});
		modalInstance.result.then(function(selectedDate) {
			$scope.filterCriteria.startDate = $filter('date')(selectedDate,
					"dd-MM-yyyy");
